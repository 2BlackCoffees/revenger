...
class Saver(GenericSaver):
    ...
    def append_connection(
        self, line: str, keep_only_unique: bool = True
    ) -> Saver:
        if not keep_only_unique or line not in self.connections:
            ...
            self.logger.log_debug(
                f"Connection {line} was skipped because it exists already."
            )
        return self
    ...
    def removed_last_line_if_same(self, line: str) -> bool:
        return_value = False
        old_value = (
            None if len(self.lines_to_save) == 0 else self.lines_to_save[-1]
        )
        ...
        self.lines_to_save.append(
            f"'Compared {line} with last element of {old_value}"
        )
        return return_value
