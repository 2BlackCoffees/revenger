...
class JavaLanguage(LanguageDependent):
    ...
    def clean_type(self, class_name: str) -> str:
        class_name = re.sub(r"[\[\{\(\<]", "_BRQTO_", class_name)
        class_name = re.sub(r"[\>\)\]\}]", "_BRQTC", class_name)
        class_name = re.sub(r"\s+extends\s+", "_XTNDS_", class_name)
        class_name = re.sub(r"\s+implements\s+", "_MPLMNTS_", class_name)
        class_name = re.sub(r"\s+", "_SPC_", class_name)
...
class Datastructure(GenericDatastructure):
    ...
    class SubDataStructure(GenericSubDataStructure):
        ...
        def add_base_class(
            ...
            if base_class in self.from_imports.keys():
                ...
                self.logger.log_debug(
                    f"  Created base class name {base_class} as is from filemodule: >{self.filemodule}< and >{base_class}<"
                )
            else:
                base_class = f"{self.filemodule}.{base_class}"
                self.logger.log_debug(
                    f"  Created base class name {base_class} from filemodule: >{self.filemodule}< and >{base_class}<"
                )
        ...
        def add_method(
            self,
            method_name: str,
            arguments_tuple: list[tuple[str, str]],
            is_private: bool,
            method_variables_tuple: list[tuple[str, str]] = None,
        ) -> None:
            arguments = [
                Datastructure.Method.ParameterType(parameter, user_type)
                for parameter, user_type in arguments_tuple
            ]
            method_variables = [
                Datastructure.Method.MethodVariable(
                    variable_name, variable_type
                )
                for variable_name, variable_type in method_variables_tuple
            ]
            self.methods.append(
                Datastructure.Method(
                    method_name, arguments, is_private, method_variables
                )
            )
        ...
        def add_variable(
            ...
            self.variables.append(
                Datastructure.Variable(
                    variable_name, variable_type, is_member, is_private
                )
            )
    ...
    def __init__(self, language_dependent: LanguageDependent, logger: Logger):
        self.class_to_datastructure: dict[
            str, Datastructure.SubDataStructure
        ] = {}
        self.filename_to_datastructure: dict[
            str, list[Datastructure.SubDataStructure]
        ] = {}
        self.namespace_to_datastructures: dict[
            str, list[Datastructure.SubDataStructure]
        ] = {}
    ...
    def set_color_class_name_list(
        self, class_name_list: list[str], color: str
    ) -> None:
        for class_name in class_name_list:
            sub_datastructure = self.get_datastructures_from_class_name(
                class_name
            )
    ...
    def get_datastructures_from_class_name(
        self, class_name: str
    ) -> Datastructure.SubDataStructure:
        if class_name in self.class_to_datastructure:
            return self.class_to_datastructure[class_name]
        self.logger.log_debug(
            f"Requested class {class_name} was not found in the datastructure"
        )
        return None
    ...
    def get_datastructures_from_namespace(
        self, namespace: str
    ) -> list[Datastructure.SubDataStructure]:
        return self.namespace_to_datastructures[namespace]

    def get_namespace_list_from_namespace_name(
    ...
    def append_sub_datastructure(
        self, sub_datastructure: Datastructure.SubDataStructure
    ) -> None:
        ...
        if fqdn_class_name not in self.class_to_datastructure.keys():
            ...
            self.namespace_to_datastructures[namespace].append(
                sub_datastructure
            )
            self.namespace_to_namespace_list[
                namespace
            ] = sub_datastructure.get_name_space_list()
        else:
            self.logger.log_debug(
                f"WARNING: Class {fqdn_class_name} is being registered a second time \n"
...
class DatastructureHandler:
    ...
    def __append_sub_datastructures_from_classname(
        self, classname: str, reduced_datastructure: Datastructure
    ) -> Datastructure.SubDataStructure:
        # type_wo_namespace: str = re.sub('^.*\.', '', classname)
        if classname not in self.datastructure.get_skip_types():
            sub_datastructure: Datastructure.SubDataStructure = (
                self.datastructure.get_datastructures_from_class_name(
                    classname
                )
            )
            if sub_datastructure is not None:
                reduced_datastructure.append_sub_datastructure(
                    sub_datastructure
                )
                self.logger.log_debug(
                    f"  Appended sub datastructure of {sub_datastructure.get_fqdn_class_name()}"
                )
                return sub_datastructure
            else:
                self.logger.log_debug(
                    f"  Could not find internal sub_datastructure for class {classname}"
                )
        else:
            self.logger.log_debug(
                f"  {classname} was skipped because it belongs to the skipped types {self.datastructure.get_skip_types()}"
            )

        return None

    def __add_class_to_reduced_datastructure_if_not_exist(
        self, class_name: str, reduced_datastructure: Datastructure
    ) -> Datastructure.SubDataStructure:
        if self.datastructure.class_exists(class_name):
            return self.__append_sub_datastructures_from_classname(
                class_name, reduced_datastructure
            )
        return None

    def create_reduced_class_list_from_class_name_list(
        self, class_name_list: list[str]
    ) -> Datastructure:
        self.datastructure.clear_color()
        reduced_datastructure: Datastructure = Datastructure(
            self.datastructure.get_language_dependent(), self.logger
        )
        self.logger.log_debug(
            f"create_reduced_class_list_from_class_name_list(class_name_list = {class_name_list})"
        )

        for class_name in class_name_list:
            self.logger.log_debug(f" Adding class {class_name}")
            sub_datastructure: Datastructure.SubDataStructure = (
                self.__add_class_to_reduced_datastructure_if_not_exist(
                    class_name, reduced_datastructure
                )
            )
            if sub_datastructure is not None:
                ...
                self.logger.log_debug(
                    f"  All base classes for class {class_name} are: {sub_datastructure.get_base_classes()})"
                )
                for base_class_name in sub_datastructure.get_base_classes():
                    ...
                    self.logger.log_debug(
                        f" Adding parent class {base_class_name} of {class_name}"
                    )
                    self.logger.log_debug(
                        f"  All reduced base classes for {class_name}: {reduced_datastructure.get_datastructures_from_class_name(class_name).get_base_classes()})"
                    )

                static_field: Datastructure.Static
                self.logger.log_debug(
                    f'  All static fields for class {class_name} are: {[f"{static_field.static_name}:{static_field.static_type}" for static_field in sub_datastructure.get_static_fields()]})'
                )
                for static_field in sub_datastructure.get_static_fields():
                    _, reduced_member_type, _ = Common.reduce_member_type(
                    ...
                    self.logger.log_debug(
                        f" Adding static related class {reduced_member_type} of {class_name}"
                    )

                variable_field: Datastructure.Variable
                self.logger.log_debug(
                    f'  All variable fields for class {class_name} are: {[f"{variable_field.variable_name}:{variable_field.variable_type}" for variable_field in sub_datastructure.get_variable_fields()]})'
                )
                for variable_field in sub_datastructure.get_variable_fields():
                    ...
                    self.logger.log_debug(
                        f" Adding variable related class {reduced_member_type} of {class_name}"
                    )

                method_field: Datastructure.Method
                self.logger.log_debug(
                    f'  All methods for class {class_name} are: {[f"{method_field.method_name}" for method_field in sub_datastructure.get_method_fields()]})'
                )
                for method_field in sub_datastructure.get_method_fields():
                    self.logger.log_debug(
                        f'    All parameters for method {class_name}.{method_field.method_name} are: {[f"{parameter.parameter}:{parameter.user_type}" for parameter in method_field.parameters]})'
                    )
                    for parameter in method_field.parameters:
                        ...
                        self.logger.log_debug(
                            f" {parameter.parameter}:{parameter.user_type} (reduced to {reduced_member_type}) is a parameter from method {method_field.method_name} from class {sub_datastructure.get_fqdn_class_name()} "
                        )
                    self.logger.log_debug(
                        f'    All local variables for method {class_name}.{method_field.method_name} are: {[f"{variable.variable_name}:{variable.variable_type}" for variable in method_field.variables]})'
                    )
                    for variable in method_field.variables:
                        ...
                        self.logger.log_debug(
                            f" {variable.variable_name}:{variable.variable_type} (reduced to {reduced_member_type}) is a variable from method {method_field.method_name} from class {sub_datastructure.get_fqdn_class_name()} "
                        )

                self.logger.log_debug(
                    f"  All inner classes for class {class_name} are: {sub_datastructure.get_inner_class_name()})"
                )
                for (
                    inner_class_name
                    ...
                    self.logger.log_debug(
                        f" Adding inner class {reduced_member_type} of {class_name}"
                    )

        self.logger.log_debug(
            f"Analyzing all classes referencing {class_name}"
        )
            ...
            for (
                ...
                for base_classname in sub_datastructure.get_base_classes():
                    if base_classname in class_name_list:
                        self.logger.log_debug(
                            f" Adding child related class {sub_datastructure.get_fqdn_class_name()} of {base_classname}"
                        )
                        self.__append_sub_datastructures_from_classname(
                            sub_datastructure.get_fqdn_class_name(),
                            reduced_datastructure,
                        )
                ...
                for static_field in sub_datastructure.get_static_fields():
                    ...
                    if reduced_member_type in class_name_list:
                        self.logger.log_debug(
                            f" Adding {sub_datastructure.get_fqdn_class_name()}: {static_field.static_name}.{reduced_member_type}  is a static related class of {sub_datastructure.get_fqdn_class_name()} "
                        )
                        self.__append_sub_datastructures_from_classname(
                            sub_datastructure.get_fqdn_class_name(),
                            reduced_datastructure,
                ...
                for variable_field in sub_datastructure.get_variable_fields():
                    ...
                    if reduced_member_type in class_name_list:
                        self.logger.log_debug(
                            f" Adding {sub_datastructure.get_fqdn_class_name()}: {variable_field.variable_name}:{variable_field.variable_type} (reduced to {reduced_member_type}) is a variable related class of {sub_datastructure.get_fqdn_class_name()} "
                        )
                        self.__append_sub_datastructures_from_classname(
                            sub_datastructure.get_fqdn_class_name(),
                            reduced_datastructure,
                ...
                for method_field in sub_datastructure.get_method_fields():
                    for parameter in method_field.parameters:
                        ...
                        if reduced_member_type in class_name_list:
                            self.logger.log_debug(
                                f" Adding {sub_datastructure.get_fqdn_class_name()}: {reduced_member_type} is a parameter from method {method_field.method_name} related class of {sub_datastructure.get_fqdn_class_name()} "
                            )
                            self.__append_sub_datastructures_from_classname(
                                sub_datastructure.get_fqdn_class_name(),
                                reduced_datastructure,
                    ...
                    for variable in method_field.variables:
                        ...
                        if reduced_member_type in class_name_list:
                            self.logger.log_debug(
                                f" Adding {sub_datastructure.get_fqdn_class_name()}: {variable.variable_name}:{variable.variable_type} (reduced to {reduced_member_type}) which is a local variable from method {method_field.method_name} from class {sub_datastructure.get_fqdn_class_name()} "
                            )
                            self.__append_sub_datastructures_from_classname(
                                sub_datastructure.get_fqdn_class_name(),
                                reduced_datastructure,
                ...
                for (
                    ...
                    if reduced_member_type in class_name_list:
                        self.logger.log_debug(
                            f" Adding {sub_datastructure.get_fqdn_class_name()}: {reduced_member_type} contains class of {sub_datastructure.get_fqdn_class_name()} "
                        )
                        self.__append_sub_datastructures_from_classname(
                            sub_datastructure.get_fqdn_class_name(),
                            reduced_datastructure,
        ...
        for namespace_name in self.datastructure.get_sorted_name_spaces():
            for (
                ...
                for namespace in namespace_list:
                    if len(full_name_space) > 0:
                        full_name_space += "."
                    full_name_space += namespace
                    if (
                        full_name_space
                        not in class_name_list_grouped_by_namespaces.keys()
                    ):
                        class_name_list_grouped_by_namespaces[
                            full_name_space
                        ] = []
        for namespace in class_name_list_grouped_by_namespaces.keys():
            for classname in self.datastructure.get_classname_list():
                if classname.startswith(namespace):
                    class_name_list_grouped_by_namespaces[namespace].append(
                        classname
                    )
