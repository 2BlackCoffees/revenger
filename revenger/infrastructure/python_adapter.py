...
class PythonAdapter:
    ...
    def get_type(
        ...
        if member_sub_type in type_dict.keys():
            ...
            self.logger.log_debug(
                f"  Created sub type {member_sub_type} from filemodule: >{filemodule}<"
            )
            self.logger.log_debug(
    ...
    @staticmethod
    def __get_namespace_name_from_filename(
        filename: str, from_dir: str
    ) -> str:
        if from_dir is not None:
            filename = filename.replace(from_dir, "")
        return re.sub(
            r"^\.", "", re.sub(r"\.py$", "", filename.replace("/", "."))
        )

    def read_python_ast(
        self, datastructure: GenericDatastructure, filename: str, from_dir: str
    ) -> any:
        ...
        filemodule: str = PythonAdapter.__get_namespace_name_from_filename(
            filename, from_dir
        )
        ...
        for node in tree.body:
            if isinstance(node, ast.ImportFrom):
                ...
                for module_name in node.names:
                    from_import[module_name.name] = (
                        module_path + "." + module_name.name
                    )

        for node in tree.body:
            if isinstance(node, ast.ClassDef):
                self.analyze_class_def(
                    node, datastructure, filename, from_import, filemodule
                )

    def analyze_class_def(
        self,
        node: ast.ClassDef,
        ...
        if parent_class_name is None:
            ...
            self.logger.log_debug(
                f"Created class_name {class_name} from filemodule: >{filemodule}< and >{node.name}<"
            )
        else:
        ...
        for class_body in node.body:
            if isinstance(class_body, ast.ClassDef):
                ...
                from_parent: str = (
                    f"{parent_class_name}.{node.name}"
                    if parent_class_name is not None
                    else node.name
                )
                self.logger.log_debug(
                    f"  Created from_parent name {from_parent}"
                )
            ...
            if isinstance(class_body, ast.AnnAssign):
                ...
                if isinstance(class_body.annotation, ast.Name):
                    static_type = self.get_type(
                        datastructure.get_skip_types(),
                        class_body.annotation.id,
                        from_import,
                        filemodule,
                    )
                    self.logger.log_debug(
                        f" Analyzing ast.Name static type {static_type} from file {filename}"
                    )
                elif isinstance(class_body.annotation, ast.Subscript):
                    if isinstance(
                        class_body.annotation.value, ast.Name
                    ) and isinstance(class_body.annotation.slice, ast.Name):
                        member_sub_type = self.get_type(
                            datastructure.get_skip_types(),
                            class_body.annotation.slice.id,
                            from_import,
                            filemodule,
                        )
                        static_type = (
                            class_body.annotation.value.id
                            + "["
                            + member_sub_type
                            + "]"
                        )
            ...
            if isinstance(class_body, ast.FunctionDef):
                ...
                if method_name != "":
                    ...
                    class_datastructure.add_method(
                        method_name,
                        [
                        ...
                        if isinstance(target, ast.Attribute) or isinstance(
                            ...
                            if isinstance(annotation, ast.Subscript):
                                if (
                                    ...
                                    and hasattr(annotation, "slice")
                                ):
                                    self.logger.log_debug(
                                        f" Subscript function type from file {filename}"
                                    )
                                    member_sub_type = self.get_type(
                                        datastructure.get_skip_types(),
                                        annotation.slice.id,
                                ...
                                self.logger.log_debug(
                                    f" Name function type from file {filename}"
                                )
                                ...
                                self.saver.append(
                                    f"'WARNING: Will not import member named {member_name}"
                                )
                            if len(member_type) > 0 and (
                                is_member
                                or member_type
                                not in datastructure.get_skip_types()
                            ):
                                self.logger.log_debug(
                                    f"   Function type from file {filename} method {method_name}, member_type: {member_type}, is_member: {is_member}"
                                )
                                class_datastructure.add_variable(
                                    member_name, member_type, is_member
                                )
